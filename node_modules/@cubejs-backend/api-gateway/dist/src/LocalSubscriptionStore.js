...
class LocalSubscriptionStore {
    ...
    async subscribe(connectionId, subscriptionId, subscription) {
        const connection = this.getConnection(connectionId);
        connection.subscriptions[subscriptionId] = Object.assign(Object.assign({}, subscription), { timestamp: new Date() });
    }
    ...
    async getAllSubscriptions() {
        return Object.keys(this.connections).map(connectionId => {
            Object.keys(this.connections[connectionId].subscriptions).filter(subscriptionId => new Date().getTime() -
                this.connections[connectionId].subscriptions[subscriptionId].timestamp.getTime() >
                this.hearBeatInterval * 4 * 1000).forEach(subscriptionId => { delete this.connections[connectionId].subscriptions[subscriptionId]; });
            return Object.keys(this.connections[connectionId].subscriptions)
                .map(subscriptionId => (Object.assign({ connectionId }, this.connections[connectionId].subscriptions[subscriptionId])));
...
//# sourceMappingURL=LocalSubscriptionStore.js.map