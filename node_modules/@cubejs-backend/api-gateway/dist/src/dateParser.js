...
function dateParser(dateString, timezone, now = new Date()) {
    ...
    if (dateString.match(/(this|last)\s+(day|week|month|year|quarter|hour|minute|second)/)) {
        ...
        momentRange = [
            ...
            moment_timezone_1.default.tz(timezone).endOf(span).add(-1, match[2])
        ];
    }
        ...
        const fromResults = chrono_node_1.default.parse(from, current.format(moment_timezone_1.default.HTML5_FMT.DATETIME_LOCAL_MS));
        const toResults = chrono_node_1.default.parse(to, current.format(moment_timezone_1.default.HTML5_FMT.DATETIME_LOCAL_MS));
        if (!fromResults) {
        ...
        const results = chrono_node_1.default.parse(dateString, moment_timezone_1.default().tz(timezone).format(moment_timezone_1.default.HTML5_FMT.DATETIME_LOCAL_MS));
        if (!results) {
