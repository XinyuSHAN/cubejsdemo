...
class BaseDriver {
   ...
   `;
    ...
    getSslOptions() {
        ...
        if (process.env.CUBEJS_DB_SSL === 'true' ||
            ...
            ssl = sslOptions.reduce((agg, { name, envKey, canBeFile, validate }) => {
                if (process.env[envKey]) {
                    ...
                    if (canBeFile) {
                        if (shared_1.isFilePath(value)) {
                            if (!fs_1.default.existsSync(value)) {
                                throw new Error(`Unable to find ${name} from path: "${value}"`);
                            }
                            return Object.assign(Object.assign({}, agg), { [name]: fs_1.default.readFileSync(value) });
                        }
                        else if (!validate(value)) {
                            throw new Error(`${envKey} is not a valid ssl key. If it's a path, please specify it correctly`);
                        }
                    }
                    return Object.assign(Object.assign({}, agg), { [name]: value });
                }
                return agg;
            }, {});
